...
const getVideoElemForUser = user => document.getElementById(user === "user_1" ? "video-1" : "video-2");
...
export const handleIceCandidate = async ({ candidate, from, to }) => {
  try {
    const peer = from ? WebSocketClient.peers.remote : WebSocketClient.peers.local;
...
export const hangup = () => {
  ...
  const streams = videos.map(video => video.srcObject)
...
export const startCall = async (from, to) => {
  try {
    ...
    const offer = await localPeer.createOffer({ offerToReceiveAudio: 1, offerToReceiveVideo: 1 });
    ...
    localPeer.onicecandidate = event => {
      if (event.candidate) {
        WebSocketClient.webSocket.send(
          JSON.stringify({ type: "ice_candidate", from, candidate: event.candidate })
        );
      }
    }
    remotePeer.addEventListener("track", event => gotRemoteStream(event, getVideoElemForUser(to)));

    remotePeer.onicecandidate = event => {
      if (event.candidate) {
        WebSocketClient.webSocket.send(
          JSON.stringify({ type: "ice_candidate", to, candidate: event.candidate })
        );
      }
    }
